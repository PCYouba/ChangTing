...
function product_del(obj, id) {
    ...
    layer.confirm('确认要删除id为' + id + '的歌手吗？', function () {
        layer.confirm('这样做同样会删除该歌手的专辑和歌曲,您确定这样做吗？', function () {
        ...
        $.ajax({
            ...
            success: function (data1,data2) {
                if (data1) {
                    layer.msg('已删除ID为' + id + '的歌手!包括' + data2 + '条相关专辑', { icon: 1, time: 1000 });
...
function datadel() {
    ...
    layer.confirm('确认要删除这' + fxkint.length + '个歌手的数据吗？', function () {
        layer.confirm('这样做同样会删除这些歌手的专辑和歌曲,您确定这样做吗？', function () {
        $.ajax({
            cache: false,
            type: 'Post',
            url: '/Singer/DeleteSingerMulti',
            data: { fxkint: fxkint },
            dataType: 'html',
