...
function product_del(obj, id) {
    layer.confirm('确认要删除id为' + id + '的专辑吗？', function () {
        layer.confirm('这样做同样会删除该专辑的歌曲,您确定这样做吗？', function () {
        $.ajax({
            cache: false,
            type: 'Post',
            ...
            success: function (data1,data2) {
                if (data1) {
                    layer.msg('已删除ID为' + id + '的专辑!包括' + data2 + '条相关歌曲', { icon: 1, time: 1000 });
...
function datadel() {
    ...
    layer.confirm('确认要删除这' + fxkint.length + '个专辑的数据吗？', function () {
        layer.confirm('这样做同样会删除这些专辑的歌曲,您确定这样做吗？', function () {
            $.ajax({
                cache: false,
                type: 'Post',
